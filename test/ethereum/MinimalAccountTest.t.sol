...
contract MinimalAccountTest is Test, ZkSyncChainChecker {
    ...
    function testEntryPointCanExecuteCommands() public skipZkSync {
        ...
        // Act
        vm.prank(randomuser); //random user sending the transactoion, and we are signing
        IEntryPoint(helperConfig.getConfig().entryPoint).handleOps(ops, payable(randomuser)); //random user getting gas back for sending the transaction
